...
export default function CreateTaskPage() {
  ...
  return (
    <div className="-m-6 flex min-h-[1000px] flex-col bg-paper">
      {/* Top breadcrumb + title */}
      <div className="flex items-end justify-between gap-4 px-6 pt-5 md:px-10">
        <div>
          <div className="mb-1 text-[11.5px] text-ink-5">
            <button
              type="button"
              onClick={() => router.push("/teacher/tasks")}
              className="cursor-pointer text-ink-5 transition-colors hover:text-ink-3"
            >
              任务
            </button>
            <span className="mx-1.5 opacity-50">/</span>
            <span className="text-ink-3">新建任务</span>
          </div>
      ...
      {/* 2-column body: stepper + main */}
      <div className="grid flex-1 grid-cols-1 gap-6 px-6 pb-10 pt-6 md:grid-cols-[220px_1fr] md:px-10">
        <WizardStepper
          step={step}
          onJump={jumpTo}
          taskType={form.taskType}
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <div className="flex items-center justify-between rounded-[10px] border border-line bg-surface px-4 py-3.5">
            ...
            <span className="text-xs text-ink-5 tabular-nums">
              {step + 1} / {WIZARD_STEPS.length} · {WIZARD_STEPS[step].label}
            </span>
